...
if [ "$MODE" = "web" ]; then
    ...
    exec su -s /bin/sh appuser -c 'gunicorn --bind 0.0.0.0:5000 --workers 2 --threads 2 --timeout 60 --worker-tmp-dir /dev/shm --control-socket /tmp/gunicorn/control.sock --access-logfile - run:app'
